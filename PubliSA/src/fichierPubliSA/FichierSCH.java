...
public class FichierSCH extends File{
	 ...
	 */
	public FichierSCH(String adresse) {
		super(adresse);
		isPresent = false;
		...
		if(nom.substring(nom.indexOf(".")+1, nom.length()).toUpperCase().contains("SCH")){
			...
			if (Integer.parseInt(planche) > 9 && Integer.parseInt(planche) < 36){       
	        	...
	        	NomTheorique = "L" + livre + "C" + chapitre + "P" + planche + "V" + variante + "I" + issue + ".SCH";
	...
	public String changeAdresse(){
		...
		if(nom.substring(nom.indexOf(".")+1, nom.length()).toUpperCase().contains("SCH")){
			...
			if(nom.substring(nom.indexOf(".")+1, nom.length()).contentEquals("sch")){
				nom = nom.substring(0, nom.lastIndexOf(".")) + ".SCH";
				adresse = repertoire + File.separator + nom;
				changement = true;
			}
			...
			if (issue.length() == 2){
				...
				nom = nom.substring(0, nom.lastIndexOf("I")) + issue + ".SCH";
				adresse = repertoire + File.separator + nom;
		        changement = true;
			}
